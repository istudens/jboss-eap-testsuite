...
public class TestXAResourceRecoveryHelper implements XAResourceRecoveryHelper {
    ...
    public boolean initialise(String param) throws Exception {
        log.info("initialise(param=" + param + ")");
    ...
    public void removeLog(Xid xid) throws XAException {
        log.info("removeLog(xid=" + xid);

        synchronized (preparedXids) {
            if (preparedXids.remove(xid)) {
                writeToDisk();
            } else {
                log.info("no log present for " + xid);
            }
        }
    }
    ...
    public Xid[] recover() {
        log.info("recover()");
        log.info("returning " + preparedXids.size() + " Xids");
    ...
    private void writeToDisk() {
        ...
        log.info("logging " + preparedXids.size() + " records to " + logFile.getAbsolutePath());
    ...
    private void recoverFromDisk() {
        File logFile = getLogFile();
        log.info("recovering from " + logFile.getAbsolutePath());
